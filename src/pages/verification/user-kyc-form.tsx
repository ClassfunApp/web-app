...
import { useSubmitUserVerification } from '../../hooks/queries/use-verification';
...
function FileInput({
  ...
  return (
    <div className="space-y-1">
      ...
      <input ref={ref} type="file" name={name} accept={accept} className="hidden" onChange={handleChange} />
    </div>
  );
}
...
export function UserKycForm({ existing, onSuccess }: Props) {
  const submit = useSubmitUserVerification();
  const [files, setFiles] = useState<Record<string, File | null>>({
    idImage: null,
    proofOfAddress: null,
  });
  const [form, setForm] = useState({
    bvnSsn: existing?.bvnSsn || '',
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Input label="BVN / SSN" placeholder="Enter your BVN or SSN" value={form.bvnSsn} onChange={set('bvnSsn')} />
          <Select
            label="ID Type"
            value={form.idType}
            onChange={set('idType')}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
          <FileInput
            ...
            name="idImage"
            currentUrl={existing?.idImageUrl}
            onChange={(f) => setFiles((prev) => ({ ...prev, idImage: f }))}
          />
          <FileInput
