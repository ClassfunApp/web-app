...
export default function TimetablePage() {
  ...
  return <div className="space-y-6 print:p-4">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="flex items-center gap-2 text-2xl font-bold text-slate-900 dark:text-white"><CalendarDays className="text-indigo-600" /> {teacherOnly ? 'My Timetable' : 'School Timetable'}</h1><p className="mt-1 text-sm text-slate-500">{teacherOnly ? 'Your assigned lessons by day and time.' : 'Plan lessons by day, time, class, teacher and room. Conflicts are blocked automatically.'}</p></div><div className="flex gap-2 print:hidden"><Button variant="secondary" onClick={() => window.print()}><Printer size={16} /> Print</Button>{!teacherOnly && <Button onClick={() => setEditing(null)} disabled={!selectedCenter}><Plus size={16} /> Add lesson</Button>}</div></div>
