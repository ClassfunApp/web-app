...
function TimetableForm({ entry, centerId, close }: { entry: TimetableEntry | null; centerId: string; close: () => void }) {
  ...
  async function submit(e: React.FormEvent) { e.preventDefault(); setError(''); try { await save.mutateAsync({ id: entry?.id, centerId, activityId, classLevelId: classLevelId || null, teacherId: teacherId || null, dayOfWeek, startTime, endTime, room: room || null, notes: notes || null }); close(); } catch (e: unknown) { const err = e as { response?: { data?: { message?: string } }; message?: string }; setError(err.response?.data?.message ?? err.message ?? 'Could not save lesson'); } }
  return <form onSubmit={submit} className="space-y-4">
    ...
    <Select label="Teacher" value={teacherId} onChange={(e) => setTeacherId(e.target.value)} options={[{ value: '', label: 'Unassigned' }, ...teachers.map((u) => ({ value: u.id, label: u.fullName }))]} />
...
export default function TimetablePage() {
  ...
  return <div className="space-y-6 print:p-4">
    ...
    {isLoading ? <Loading /> : <div className="grid gap-4 xl:grid-cols-5">{DAYS.map((day, index) => <Card key={day} className="min-w-0"><h2 className="mb-3 font-bold text-slate-800 dark:text-slate-100">{day}</h2><div className="space-y-2">{grouped[index].map((entry) => <div key={entry.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-800"><p className="text-xs font-bold text-indigo-600">{entry.startTime.slice(0, 5)}–{entry.endTime.slice(0, 5)}</p><p className="mt-1 font-semibold text-slate-800 dark:text-white">{entry.activity?.name}</p><p className="text-xs text-slate-500">{entry.classLevel?.name ?? 'All classes'} · {entry.room || 'Room TBA'}</p><p className="text-xs text-slate-500">{entry.teacher?.fullName ?? 'Teacher TBA'}</p><div className="mt-2 flex justify-end gap-1 print:hidden"><button aria-label="Edit" className="p-1 text-slate-400 hover:text-indigo-600" onClick={() => setEditing(entry)}><Pencil size={14} /></button><button aria-label="Delete" className="p-1 text-slate-400 hover:text-red-600" onClick={() => { if (confirm('Delete this lesson?')) remove.mutate(entry.id); }}><Trash2 size={14} /></button></div></div>)}{!grouped[index].length && <p className="py-6 text-center text-xs text-slate-400">No lessons</p>}</div></Card>)}</div>}
